module gbe_control(
    ...
    always @(posedge clk or negedge a_sync_nrst) begin
        if(!a_sync_nrst)begin
            ...
            counter <= tx_en0 && counter < decimation_factor  ? counter +1 : 0;
            //Condição para envio de dados
            if(tx_en0 && counter_tx_package < tx_package_len && ena_dec )begin
                //Incrementa se o tx_en0 e ena_decimation forem 1, ena_decimation define a taxa de envio
    ...
    assign tx_eof = counter_tx_package == tx_package_len -1 && tx_en0;
    //Lógica do Valid 
    assign tx_valid = tx_en0 && ena_dec && 4'hff;
    
    //assign tx_valid = tx_en0 && counter_tx_package < tx_package_len && ena_dec;
