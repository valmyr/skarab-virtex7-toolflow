...
always@(posedge clk, negedge a_sync_nrst)begin
    if(!a_sync_nrst)begin
        ...
        if(in_valid_rx)begin
            mem[addr_data_local_mux_r]  <= current_state_rx == RX_DATA ?  in_data_rx_ethernet: mem[addr_data_local_mux_r];
            ...
            counter       <= counter < package_size - CMD_SIZE -2? counter:      0;
        ...
        if(ena_dec)begin
            ...
            data_out_buffer <= current_state_tx == TX_DATA ? mem[addr_data_local] : 0;
            addr_data_local <= current_state_tx == TX_DATA ? addr_data_local+1 : 0;
...
reg [7:0] addr_data_t;
    ...
    always@(*)begin
        case(current_state_tx)
            ...
            TX_DATA:begin
                tx_ena_out_w = 1;
                case(addr_data_local < package_size)
                    1'b1:next_state_tx = TX_DATA;
                    1'b0:next_state_tx = IDLE; 
                    default:next_state_tx = IDLE;
                endcase
...
assign debug_local_data = mem[counter];
    ...
    assign ena_dec       =  counter_dec == 15-1 | sync_dec ;
